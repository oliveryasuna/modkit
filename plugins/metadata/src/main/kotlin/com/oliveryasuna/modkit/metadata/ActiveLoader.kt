...
/**
 * Resolves the active loader from the `modkit.loader` Gradle property. The
 * generate task must be chosen eagerly in `apply()`, before the `modkit` model
 * DSL runs, so this reads a property rather than the model.
 */
